module.exports = function(app, db) {
    ...
    app.get('/api/garments', async function(req, res) {
        ...
        if (season && gender) garments = await db.many('select * from garment where season = $1 AND gender = $2', [season, gender])
        else if (gender) garments = await db.many('select * from garment where gender = $1', [gender])
        else if (season) garments = await db.many('select * from garment where season = $1', [season]);
